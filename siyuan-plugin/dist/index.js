...
__export(index_exports, {
  default: () => TempoSyncPlugin
});
module.exports = __toCommonJS(index_exports);
var import_siyuan2 = require("siyuan");

// src/storage.ts
var STORAGE_KEY = "tempo_siyuan_auth";
...
// src/config.ts
var SUPABASE_URL = "https://uvgxdmmyyujfvrvfrpsj.supabase.co" ? "https://uvgxdmmyyujfvrvfrpsj.supabase.co" : LOCAL_SUPABASE_URL;
var SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV2Z3hkbW15eXVqZnZydmZycHNqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE3NzU2MDYsImV4cCI6MjA5NzM1MTYwNn0.s1Ts2DqOf98E01Ml5WEhf6w1nLJfGo5CX5u2AOGFZTI" ? "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV2Z3hkbW15eXVqZnZydmZycHNqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE3NzU2MDYsImV4cCI6MjA5NzM1MTYwNn0.s1Ts2DqOf98E01Ml5WEhf6w1nLJfGo5CX5u2AOGFZTI" : LOCAL_ANON_KEY;
var PAIRING_ENDPOINT = "https://uvgxdmmyyujfvrvfrpsj.supabase.co/functions/v1/siyuan-pairing" ? "https://uvgxdmmyyujfvrvfrpsj.supabase.co/functions/v1/siyuan-pairing" : `${SUPABASE_URL}/functions/v1/siyuan-pairing`;
var PLUGIN_VERSION = "0.1.0" ? "0.1.0" : "0.1.0";
...
async function withAuthRetry(action) {
  try {
    return await action();
  } catch (error) {
...
async function createTaskFromSiyuan(title, siyuanBlockId, description, dueDate, priority) {
...
// src/binding.ts
async function reportPaired() {
  const supabase = getClient();
  if (!supabase) return;
  const { error } = await supabase.rpc("upsert_siyuan_binding", {
    p_paired_at: (/* @__PURE__ */ new Date()).toISOString(),
...
async function reportSync(importedCount) {
  const supabase = getClient();
  if (!supabase) return;
  const { error } = await supabase.rpc("upsert_siyuan_binding", {
    p_last_sync_at: (/* @__PURE__ */ new Date()).toISOString(),
...
async function siyuanRequest(endpoint, data = {}) {
  const result = await (0, import_siyuan.fetchSyncPost)(endpoint, data);
  if (result.code !== 0) {
...
async function getCurrentDocId() {
  try {
    const data = await siyuanRequest(
...
async function getChildBlocks(docId) {
  return siyuanRequest("/api/block/getChildBlocks", {
    id: docId
  });
}
async function getBlockAttrs(blockId) {
  return siyuanRequest("/api/attr/getBlockAttrs", {
    id: blockId
  });
}
...
function extractUncompletedTasks(blocks) {
  const tasks = [];
...
function isUncompletedTask(content) {
  return /\* \[ \]|^- \[ \]|^☐|^\* ☐/.test(content.trim()) || content.includes("\u2610") || content.includes("[ ]") && !content.includes("[x]");
}
function extractTaskTitle(content) {
  let title = content.replace(/^\* \[ \]\s*/, "").replace(/^- \[ \]\s*/, "").replace(/^☐\s*/, "").replace(/^\* ☐\s*/, "").trim();
  title = title.replace(/\*\*/g, "").replace(/\*/g, "").replace(/`/g, "");
  return title;
}
async function hasTempoId(blockId) {
  try {
...
async function writeTempoId(blockId, taskId) {
...
function renderUnpairedUI(container) {
  const title = document.createElement("h3");
  title.textContent = "\u7ED1\u5B9A Tempo \u8D26\u53F7";
  title.style.margin = "0 0 12px 0";
  container.appendChild(title);
  const hint = document.createElement("p");
  hint.textContent = "\u5728 Tempo App\uFF1A\u6211\u7684 \u2192 \u601D\u6E90\u8FDE\u63A5 \u2192 \u751F\u6210\u914D\u5BF9\u7801\uFF0C\u7136\u540E\u5728\u6B64\u8F93\u5165 6 \u4F4D\u7801";
...
function renderPairedUI(container) {
  const auth = loadAuth();
  const title = document.createElement("h3");
...
function renderPairingUI(container) {
  container.innerHTML = "<p>\u914D\u5BF9\u4E2D...</p>";
}
function renderScanningUI(container) {
  container.innerHTML = "<p>\u626B\u63CF\u4E2D...</p>";
}

// src/index.ts
var TempoSyncPlugin = class extends import_siyuan2.Plugin {
  constructor() {
    super(...arguments);
